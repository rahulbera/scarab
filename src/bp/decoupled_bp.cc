...
Flag pop_fetch_queue(uns proc_id) {
  ...
  if(!fetch_queue[proc_id].empty()) {
    ...
    if(fetch_queue[proc_id].front().op->table_info->cf_type) {
      ...
      if(USE_LATE_BP ? fetch_queue[proc_id].front().op->oracle_info.late_pred :
                       fetch_queue[proc_id].front().op->oracle_info.pred) {
...
void update_decoupled_bp() {
  ...
  switch(dbp->state) {
    case BP_NORMAL: {
      ...
      if(fetch_queue[dbp->proc_id].size() == FETCH_QUEUE_SIZE ||
         dbp->num_taken_branches_in_fetch_queue == FETCH_QUEUE_NUM_TAKEN) {
...
Bp_State cycle_decoupled_bp(uns proc_id) {
  ...
  while(break_predict == BP_NO_BREAK) {
    ...
    DEBUG(dbp->proc_id,
          "Fetching op from Decoupled BP addr: %s off: %d inst_info: %p "
          "ii_addr: %s "
          "dis: %s, uid:%llu opnum: (%s:%s)\n",
          hexstr64s(op->inst_info->addr), op->off_path, op->inst_info,
          hexstr64s(op->inst_info->addr), disasm_op(op, TRUE), op->inst_uid,
    ...
    if(op->table_info->cf_type) {
      ...
      if(IS_CALLSYS(op->table_info) || op->table_info->bar_type & BAR_FETCH) {
        ...
        dbp->next_addr            = ADDR_PLUS_OFFSET(dbp->curr_addr,
      ...
      if(break_predict == BP_NO_BREAK) {
        if(op->oracle_info.btb_miss) {
          ...
          //   dbp->next_addr = ADDR_PLUS_OFFSET(dbp->curr_addr,
          //   op->inst_info->trace_info.inst_size); DEBUG(0, "dbp next addr
          //   after btb miss is %llx\n", dbp->next_addr);
          // }
          // else{
          ...
          if(op->oracle_info.early_late_disagree) {
            ...
            // ASSERT(dbp->proc_id, dbp->next_addr ==
            // op->oracle_info.late_pred_npc); if(!op->off_path){
      ...
      // bool final_mispred = op->oracle_info.pred != op->oracle_info.dir &&
      // (dbp->next_addr != op->oracle_info.npc); bool final_misfetch =
      // !final_mispred && (dbp->next_addr != op->oracle_info.npc) &&
      ...
      if(TAKEN_PER_CYCLE && taken_count == TAKEN_PER_CYCLE &&
         break_predict == BP_NO_BREAK) {
      ...
      if(CFS_PER_CYCLE && cf_num > CFS_PER_CYCLE &&
         break_predict == BP_NO_BREAK) {
    ...
    ops_count++;
    if(BP_OP_PER_CYCLE && ops_count >= BP_OP_PER_CYCLE &&
       break_predict == BP_NO_BREAK) {
      break_predict = BP_BREAK_ON_NUM_OP;
    }
    if(fetch_queue[dbp->proc_id].size() >= FETCH_QUEUE_SIZE &&
    ...
    if(dbp->num_taken_branches_in_fetch_queue >= FETCH_QUEUE_NUM_TAKEN &&
       break_predict == BP_NO_BREAK) {
      break_predict = BP_BREAK_ON_FULL_FETCH_QUEUE;
    }
  }  // end of while loop

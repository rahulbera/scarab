...
class Stats {
 ...
 public:
  bool is_empty() { return _registry.size() == 0; }
};
...
class StatWrapper : public StatWrapperBase {
 ...
 public:
  StatWrapper(T& val, const Implementation& impl, Stats& stats) :
      _ref(&val), _impl(impl), _stats(stats) {};
  StatWrapper(std::vector<T>& val, const Implementation& impl, Stats& stats) :
      _ref(&val), _impl(impl), _stats(stats) {};
  ...
  StatWrapper& name(std::string name) {
    ...
    if(auto it = _stats._registry.find(name); it != _stats._registry.end()) {
      throw ConfigurationError(
        "Stat {} of implementation is already registered!", name);
    }

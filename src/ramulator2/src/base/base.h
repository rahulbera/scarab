...
/**
 * @brief     Base class for concrete implementation of an interface in
 * Ramulator.
 *
 * @details
 * A common base class for concrete implementations of interfaces in Ramulator.
 * An implementation of an interface should inherit from both its corresponding
 * interface class and this class.
 *
 */
class Implementation {
 ...
 protected:
  ...
  std::string m_id;  // Identifier to distinguish between multiple instances of
                     // the same implementation.

  Implementation* m_parent;  // The pointer to the parent implementation object
  std::vector<Implementation*> m_children;  // A vector of pointers to the child
 ...
 public:
  Implementation(const YAML::Node& config, std::string ifce_name,
                 std::string name, std::string desc, Implementation* parent) :
      m_config(config), m_ifce_name(ifce_name), m_name(name), m_desc(desc),
      m_id(config["id"].as<std::string>("_default_id")), m_parent(parent),
      m_params(config) {};

  Implementation(const YAML::Node& config, std::string ifce_name,
                 std::string name, std::string desc, std::string id,
                 Implementation* parent) :
      m_config(config), m_ifce_name(ifce_name), m_name(name), m_desc(desc),
      m_id(id), m_parent(parent), m_params(config) {};
  ...
  /**
   * @brief     Performs initialization of the implementation object with the
  ...
  /**
   * @brief     Setup the implementation that depends on other parts of the
   * system.
   *
   */
  ...
  Implementation_t* create_child_impl(const YAML::Node& config) {
    ...
    if(impl == nullptr) {
      throw ConfigurationError("Failed to convert  {}!",
                               Interface_t::get_name());
      return nullptr;
    }
    return impl;
  ...
  _ParamChainer<T> param(std::string param_name) {
    return m_params._param<T>(param_name);
  };
  template <typename T>
  _ParamChainer<T> param(std::string_view param_name) {
    return param<T>(std::string(param_name));
  };
  template <typename T>
  _ParamChainer<T> param(const char* param_name) {
    return param<T>(std::string(param_name));
  };

  _ParamGroupChainer param_group(std::string group_name) {
    return m_params._group(group_name);
  };

  template <typename T>
  StatWrapper<T>& register_stat(T& val) {
    StatWrapper<T>* s = new StatWrapper<T>(val, *this, m_stats);
    return *s;
  };
  template <typename T>
  StatWrapper<T>& register_stat(std::vector<T>& val) {
    StatWrapper<T>* s = new StatWrapper<T>(val, *this, m_stats);
    return *s;
  };
 ...
 private:
  template <class Interface_t>
  Interface_t* create_child(const YAML::Node& config,
                            std::string       desired_impl_name) {
    ...
    if(!child_config) {
      throw ConfigurationError(
        "Interface {} is not found in the configuration!", ifce_name);
      return nullptr;
    }
    ...
    if(impl_name == "") {
      throw ConfigurationError("No implementation specified for interface {}!",
                               ifce_name);
      return nullptr;
    }
    if(desired_impl_name != "" && desired_impl_name != impl_name) {
      throw ConfigurationError(
        "Specified implementation {} is different from the desired {}!",
        impl_name, desired_impl_name);
      return nullptr;
    }

    Implementation* impl = Factory::create_implementation(ifce_name, impl_name,
                                                          config, this);
    add_child(impl);
    ...
    if(dynamic_cast<Interface_t*>(impl)) {
      ...
      throw ConfigurationError(
        "Could not convert a pointer to {} to a pointer to {}!", impl_name,
        ifce_name);
      return nullptr;
    }
  }
...
#define RAMULATOR_REGISTER_IMPLEMENTATION(_ifce_class, _impl_class, _name,    \
 ...
 protected:                                                                   \
  ...
  _impl_class(const YAML::Node& config, Implementation* parent) :             \
      Implementation(config, _ifce_class::get_name(), _name, _desc, parent) { \
    _ifce_class::m_impl = this;                                               \
    m_params.set_impl_name(_name);                                            \
    init();                                                                   \
  };                                                                          \
  static Implementation* make_##_impl_class(const YAML::Node& config,         \
                                            Implementation*   parent) {         \
    return new _impl_class(config, parent);                                   \
  };                                                                          \
  static inline bool registered = Factory::register_implementation(           \
    _ifce_class::get_name(), _name, _desc, make_##_impl_class);
...
class TopLevel {
 ...
 public:
  void gather_components() {
    ...
    if(impl == nullptr) {
      throw ConfigurationError(
        "Error converting a frontend interface to an implementation!");
    }

...
struct Request {
  ...
  int command = -1;        // The command that need to be issued to progress the
                           // request
  int final_command = -1;  // The final command that is needed to finish the
                           // request
  bool is_stat_updated = false;  // Memory controller stats

  Clk_t arrive = -1;  // Clock cycle when the request arrive at the memory
                      // controller
  Clk_t depart = -1;  // Clock cycle when the request depart the memory
                      // controller

...
class Factory {
  using Constructor_t =
    std::function<Implementation*(const YAML::Node&, Implementation*)>;
 ...
 public:
   ...
   * @return false  Registration failed, is another interface with the same name
   * already exists?
   */
  ...
  /**
   * @brief     Registers an implementation class of an interface to the
   * registry.
   *
   * @return true   Registration sucessful.
   * @return false  Registration failed, is another interface with the same name
   * already exists?
   */
  static bool register_implementation(std::string          ifce_name,
                                      std::string          impl_name,
                                      std::string          impl_desc,
                                      const Constructor_t& cstr);

  /**
   * @brief     Construct an implementation object given the name of the
   * implementation.
   *
   */
  static Implementation* create_implementation(std::string       ifce_name,
                                               std::string       impl_name,

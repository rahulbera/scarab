...
/**
 * @brief Convert a timing constraint in nanoseconds into number of cycles
 * according to JEDEC convention.
 *
...
/**
 * @brief Convert a timing constraint in nanoseconds into number of cycles
 * according to JEDEC DDR5 convention.
 *
 * @param t_ns      Timing constraint in nanoseconds
 * @param tCK_ps    Clock cycle in picoseconds
 * @return uint64_t Number of cycles
...
Integral_t calc_log2(Integral_t val) {
  static_assert(std::is_integral_v<Integral_t>,
                "Only integral types are allowed for bitwise operations!");
...
/**
 * @brief Slice the lest significant num_bits from addr and return these bits.
 * The originial addr value is modified.
 *
 * @tparam Integral_t
 * @param addr
 * @param num_bits

...
int main(int argc, char* argv[]) {
  ...
  program.add_argument("-c", "--config")
    .metavar("\"dumped YAML configuration\"")
    .help("String dump of the yaml configuration.");
  program.add_argument("-f", "--config_file")
    .metavar("path-to-configuration-file")
    .help("Path to a YAML configuration file.");
  program.add_argument("-p", "--param")
    .metavar("KEY=VALUE")
    .append()
    .help("Specify parameter to override in the configuration file. Repeat "
          "this option to change multiple parameters.");
  ...
  if(use_dumped_yaml && has_param_override) {
    spdlog::warn("Using dumped configuration. Parameter overrides with "
                 "-p/--param will be ignored!");
  }
  ...
  // Instaniate the frontend of the simulated system, this is one of the
  // top-level objects in Ramulator 2.0. It also recursively instaniate all
  // components in the frontend.
  auto frontend = Ramulator::Factory::create_frontend(config);
  // Instaniate the memory system of the simulated system, this is one of the
  // top-level objects in Ramulator 2.0 It also recursively instaniate all
  ...
  // so that they can get each other's parameters (if needed) after their
  // initialization

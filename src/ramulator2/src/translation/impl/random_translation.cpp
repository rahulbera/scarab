...
class RandomTranslation : public ITranslation, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(
    ITranslation, RandomTranslation, "RandomTranslation",
 ...
 protected:
  ...
  Translation_t m_translation;  // A vector of <vpn:ppn> maps, each core has its
                                // own map
 ...
 public:
  void init() override {
    int seed =
      param<int>("seed")
        .desc(
          "The seed for the random number generator used to allocate pages.")
        .default_val(123);
    ...
    m_max_paddr = param<Addr_t>("max_addr")
                    ...
                    .required();
    m_pagesize =
      param<Addr_t>("pagesize_KB").desc("Pagesize in KB.").default_val(4) << 10;
  ...
  bool translate(Request& req) override {
    ...
    if(target == core_translation.end()) {
      ...
      if(m_num_free_physical_pages == 0) {
        // We run out of physical pages. Randomly replace a previously assigned
        // page (swap latency not modeled!)
        ...
        m_logger->warn("Swapping out PPN {} for Addr {}, VPN {}.",
                       ppn_to_replace, req.addr, vpn);
      } else {
    ...
    Addr_t p_addr = (core_translation[vpn] << m_offsetbits) |
                    (req.addr & ((1 << m_offsetbits) - 1));

    DEBUG_LOG(DTRANSLATE, m_logger,
              "Translated Addr {}, VPN {} to Addr {}, PPN {}.", req.addr, vpn,
